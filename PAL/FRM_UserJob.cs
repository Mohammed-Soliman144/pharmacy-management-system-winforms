...
{
    public partial class FRM_UserJob : Form
    {
        ...
        /// <summary>
        /// Method To Set Properties.Settings.Default.UserJob Is DataSource of Listbox 
        ...
        {
            //if Properties.Settings.Default.UserJobs is empty ==> stop to executing block code
            if (Properties.Settings.Default.UserJobs == null) return;
            ...
            //Properties.Settings.Default.UserJob.Cast<Generic Class Name is string >.ToList();
            listBoxJobs.DataSource = Properties.Settings.Default.UserJobs.Cast<string>().ToList();
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtEdit.Text != string.Empty && !listBoxJobs.Items.Contains(txtEdit.Text))
                {
                    //If Settings Variable equal null
                    if (Properties.Settings.Default.UserJobs == null)
                    {
                        //So add items to listbox for first time
                        listBoxJobs.Items.Add(txtEdit.Text);
                        //clear textbox
                        txtEdit.Clear();
                        //Focus on textbox
                        txtEdit.Focus();
                    ...
                    else // else if settings variable is filled 
                    {
                        ...
                        Properties.Settings.Default.UserJobs.Add(txtEdit.Text);
                        //clear textbox
                        txtEdit.Clear();
                        //Focus on textbox
                        txtEdit.Focus();
                ...
                else
                {
                    ...
                    NotificationSMS.NotifyShow("تم إضافة هذة الوظيفة من قبل", "تنبية",
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (listBoxJobs.Items.Count == 0) return;

                //loop by for in all items in listbox
                for (int i = 0; i <= listBoxJobs.Items.Count - 1; i++)
                {
                    //if listbox.selectedIndex equal index of loop
                    if (listBoxJobs.SelectedIndex == i)
                    {
                        //if Delete for first time delete from listbox
                        if (Properties.Settings.Default.UserJobs == null) listBoxJobs.Items.RemoveAt(i);
                        //for second time Delete from Settings Variable
                        else Properties.Settings.Default.UserJobs.RemoveAt(i);
                    }
                }
                //Save any edit to Settings Variable ==> its necessary
                Properties.Settings.Default.Save();
        ...
        //txtEdit_KeyPress Method to check validation Input Entry
        private void txtEdit_KeyPress(object sender, KeyPressEventArgs e)
        {
            // if user input entry is not equal letter and backspace and space 
            if (!char.IsLetter(e.KeyChar) && e.KeyChar != (char)Keys.Back && e.KeyChar != (char)Keys.Space)
            {
                //Permit user to entry
                e.Handled = true;
            }
        }
        ...
        //FRM_UserJob_FormClosing Method to Save Settings Variable before Closed form in event form.FormClosing
        private void FRM_UserJob_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                //fill items of listbox in Settings Variable
                //Properties.Settings.Default.UserJobs = casting items in listbox <string>().ToList();
                Properties.Settings.Default.UserJobs = listBoxJobs.Items.Cast<string>().ToList();
