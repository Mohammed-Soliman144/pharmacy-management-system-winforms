...
{
    public partial class FRM_SupplierSize : Form
    {
        ...
        //Constructor of FRM_SupplierSize
        public FRM_SupplierSize()
        {
            InitializeComponent();
        ...
        /// <summary>
        /// Method To Set Properties.Settings.Default.SupplierGroups Is DataSource of Listbox 
        ...
        {
            //if Properties.Settings.Default.SupplierSizes is empty ==> stop to executing block code
            if (Properties.Settings.Default.SupplierSizes == null) return;
            ...
            //Properties.Settings.Default.SupplierSizes.Cast<Generic Class Name is string >.ToList();
            listBoxSizes.DataSource = Properties.Settings.Default.SupplierSizes.Cast<string>().ToList();
            //Set Selected Index of item in listbox if zero select first item if -1 no any element selecting equal empty
            listBoxSizes.SelectedIndex = -1;
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtEdit.Text != string.Empty && !listBoxSizes.Items.Contains(txtEdit.Text))
                {
                    //If Settings Variable equal null
                    if (Properties.Settings.Default.SupplierSizes == null)
                    {
                        //So add items to listbox for first time
                        listBoxSizes.Items.Add(txtEdit.Text);
                        //clear textbox
                        txtEdit.Clear();
                        //Focus on textbox
                        txtEdit.Focus();
                    ...
                    else // else if settings variable is filled 
                    {
                        ...
                        Properties.Settings.Default.SupplierSizes.Add(txtEdit.Text);
                        //clear textbox
                        txtEdit.Clear();
                        //Focus on textbox
                        txtEdit.Focus();
                ...
                else
                {
                    ...
                    NotificationSMS.NotifyShow("تم إضافة هذا النوع من قبل", "تنبية",
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (listBoxSizes.Items.Count == 0) return;

                //loop by for in all items in listbox
                for (int i = 0; i <= listBoxSizes.Items.Count - 1; i++)
                {
                    //if listbox.selectedIndex equal index of loop
                    if (listBoxSizes.SelectedIndex == i)
                    {
                        //if Delete for first time delete from listbox
                        if (Properties.Settings.Default.SupplierSizes == null) listBoxSizes.Items.RemoveAt(i);
                        //for second time Delete from Settings Variable
                        else Properties.Settings.Default.SupplierSizes.RemoveAt(i);
                    }
                }
                //Save any edit to Settings Variable ==> its necessary
                Properties.Settings.Default.Save();
        ...
        {
            //Focus On comboSize
            FRM_Suppliers.SupAccess_Property.comboSize.Focus();
            //Close Form
            this.Close();
        }
        ...
        //FRM_SupplierSize_FormClosing Method to Save Settings Variable before Closed form in event form.FormClosing
        private void FRM_SupplierSize_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                //fill items of listbox in Settings Variable
                //Properties.Settings.Default.SupplierSizes = casting items in listbox <string>().ToList();
                Properties.Settings.Default.SupplierSizes = listBoxSizes.Items.Cast<string>().ToList();
