...
{
    public partial class FRM_SupplierGroup : Form
    {
        ...
        //Constructor of FRM_SupplierGroup
        public FRM_SupplierGroup()
        {
            InitializeComponent();
        ...
        {
            //if Properties.Settings.Default.SupplierGroups is empty ==> stop to executing block code
            if (Properties.Settings.Default.SupplierGroups == null) return;
            ...
            //Properties.Settings.Default.SupplierGroups.Cast<Generic Class Name is string >.ToList();
            listBoxGroups.DataSource = Properties.Settings.Default.SupplierGroups.Cast<string>().ToList();
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtEdit.Text != string.Empty && !listBoxGroups.Items.Contains(txtEdit.Text))
                {
                    //If Settings Variable equal null
                    if (Properties.Settings.Default.SupplierGroups == null)
                    {
                        //So add items to listbox for first time
                        listBoxGroups.Items.Add(txtEdit.Text);
                        //clear textbox
                        txtEdit.Clear();
                        //Focus on textbox
                        txtEdit.Focus();
                    ...
                    else // else if settings variable is filled 
                    {
                        ...
                        Properties.Settings.Default.SupplierGroups.Add(txtEdit.Text);
                        //clear textbox
                        txtEdit.Clear();
                        //Focus on textbox
                        txtEdit.Focus();
                ...
                else
                {
                    ...
                    NotificationSMS.NotifyShow("تم إضافة هذة المجموعة من قبل", "تنبية",
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i <= listBoxGroups.Items.Count - 1; i++)
                {
                    //if listbox.selectedIndex equal index of loop
                    if (listBoxGroups.SelectedIndex == i)
                    {
                        //if Delete for first time delete from listbox
                        if (Properties.Settings.Default.SupplierGroups == null) listBoxGroups.Items.RemoveAt(i);
                        //for second time Delete from Settings Variable
                        else Properties.Settings.Default.SupplierGroups.RemoveAt(i);
                    }
                }
                //Save any edit to Settings Variable ==> its necessary
                Properties.Settings.Default.Save();
        ...
        {
            //Focus On comboGroup
            FRM_Suppliers.SupAccess_Property.comboGroup.Focus();
            //Close Form
            this.Close();
        }
        ...
        //FRM_SupplierGroup_FormClosing Method to Save Settings Variable before Closed form in event form.FormClosing
        private void FRM_SupplierGroup_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                //fill items of listbox in Settings Variable
                //Properties.Settings.Default.SupplierGroups = casting items in listbox <string>().ToList();
                Properties.Settings.Default.SupplierGroups = listBoxGroups.Items.Cast<string>().ToList();
