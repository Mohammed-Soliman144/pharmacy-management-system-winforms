...
{
    public partial class FRM_ItemClassOfActions : Form
    {

        #region Public Declarations
        //Create New Instance From BAL.CLS_ItemClassOfActions Business Access Layer
        readonly BAL.CLS_ItemClassOfActions ClassOfActions = new BAL.CLS_ItemClassOfActions();
        ...
        private void InitializeDGV()
        {
            ...
            dgvActions.AllowUserToOrderColumns = false;
            //Intialize AllowUserToResizeColumns of DGV is False
            dgvActions.AllowUserToResizeColumns = false;
            //Intialize AutoSizeColumnsMode of DGV is DataGridViewAutoSizeColumnsMode.DisplayCells
            dgvActions.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.DisplayedCells;
            //Intialize AutoSizeRowsMode of DGV is DataGridviewAutoSizeRowsMode.DisplayedCells
            dgvActions.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.DisplayedCells;
            //Intialize AutoGenerateColumns of DGV is False
            dgvActions.AutoGenerateColumns = false;
            //Intialize DataSource of DGV is ActionsFormClass.ShowItemActionsTable();
            dgvActions.DataSource = ClassOfActions.ShowItemClassTable();
        }
        ...
        private void ClearControls()
        {
            ...
            dgvActions.DataSource = ClassOfActions.ShowItemClassTable();
            //Clear Text Box
            txtActionsName.Clear();
            //Focus Of Text Box
            txtActionsName.Focus();
        }

        //AddItemActionsForm Method To Add New Actions Form to ItemActionsForm Table
        private void AddItemActionsForm()
        {
            try
            {
                //Check all required fields are filled or not
                if (txtActionsName.Text == string.Empty)
                {
                ...
                //check Duplicate of item Actions Form in datagridview by for loop
                for (int i = 0; i <= dgvActions.Rows.Count - 1; i++)
                {
                    //if dgvActions.Rows[i].Cells[1].Value.ToString() is equal txtActionsName.Text
                    if (dgvActions.Rows[i].Cells[1].Value.ToString() == txtActionsName.Text)
                    {
                        //Warning Message Box
                ...
                ClassOfActions.SaveClassOfAction(txtActionsName.Text, IP_PCName, Program.UsrID, true, DateTime.Now.ToString("yyyy-MM-dd"),
        ...
        //dgvActions_CellContentClick Method to Delete any record of ItemActions Table
        private void dgvActions_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.RowIndex >= 0 && e.ColumnIndex == 3)
                {
                    ...
                    //if user select Yes Button and Select Delete itemActions From Database
                    if (NotificationSMS.NotifyResult == true && NotificationSMS.NotifyType == "Delete Record")
                    {
                        //Delete itemActions From Database
                        ClassOfActions.DeleteClassOfAction(Convert.ToInt32(dgvActions.CurrentRow.Cells[0].Value));
                    ...
                    //if user select Yes Button and Select InActivate itemActionsForm From Database
                    else if (NotificationSMS.NotifyResult == true && NotificationSMS.NotifyType == "InActivate Record")
                    {

                        //Update Status to False in itemActions Table
                        ClassOfActions.DeactivateClassOfAction(Convert.ToInt32(dgvActions.CurrentRow.Cells[0].Value), false);
        ...
        //txtActionsName_KeyDown to Recall Method btnSave_Click(sender, e); 
        private void txtActionsName_KeyDown(object sender, KeyEventArgs e)
        {
            //if e.KeyCode == Keys.Enter
            if (e.KeyCode == Keys.Enter)
            {
                //Add New Actions Form to ItemActionsForm Table
                AddItemActionsForm();
            }
        }
