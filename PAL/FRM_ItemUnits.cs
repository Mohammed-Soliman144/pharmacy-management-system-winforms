...
{
    public partial class FRM_ItemUnits : Form
    {

        #region Public Declarations
        //Create New Instance From BAL.CLS_ItemUnits Business Access Layer
        readonly BAL.CLS_ItemUnits UnitClass = new BAL.CLS_ItemUnits();
        ...
        private void InitializeDGV()
        {
            ...
            dgvUnit.AllowUserToOrderColumns = false;
            //Intialize AllowUserToResizeColumns of DGV is False
            dgvUnit.AllowUserToResizeColumns = false;
            //Intialize AutoSizeColumnsMode of DGV is DataGridViewAutoSizeColumnsMode.DisplayCells
            dgvUnit.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.DisplayedCells;
            //Intialize AutoSizeRowsMode of DGV is DataGridviewAutoSizeRowsMode.DisplayedCells
            dgvUnit.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.DisplayedCells;
            //Intialize AutoGenerateColumns of DGV is False
            dgvUnit.AutoGenerateColumns = false;
            //Intialize DataSource of DGV is UnitFormClass.ShowItemUnitTable();
            dgvUnit.DataSource = UnitClass.ShowItemUnitTable();
        }
        ...
        //AddItemUnitForm Method To Add New Unit Form to ItemUnitForm Table
        private void AddItemUnitForm()
        {
            try
            {
                ...
                //check Duplicate of item Unit Form in datagridview by for loop
                for (int i = 0; i <= dgvUnit.Rows.Count - 1; i++)
                {
                    //if dgvUnit.Rows[i].Cells[1].Value.ToString() is equal txtUnitName.Text
                    if (dgvUnit.Rows[i].Cells[1].Value.ToString() == txtUnitName.Text)
                    {
                        //Warning Message Box
                ...
                UnitClass.SaveUnit(txtUnitName.Text, IP_PCName, Program.UsrID, true, DateTime.Now.ToString("yyyy-MM-dd"),
        ...
        //dgvUnit_CellContentClick Method to Delete any record of ItemUnit Table
        private void dgvUnit_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.RowIndex >= 0 && e.ColumnIndex == 3)
                {
                    ...
                    //if user select Yes Button and Select Delete itemUnit From Database
                    if (NotificationSMS.NotifyResult == true && NotificationSMS.NotifyType == "Delete Record")
                    {
                        //Delete itemUnit From Database
                        UnitClass.DeleteUnit(Convert.ToInt32(dgvUnit.CurrentRow.Cells[0].Value));
                    ...
                    //if user select Yes Button and Select InActivate itemUnitForm From Database
                    else if (NotificationSMS.NotifyResult == true && NotificationSMS.NotifyType == "InActivate Record")
                    {

                        //Update Status to False in itemUnit Table
                        UnitClass.DeactivateUnit(Convert.ToInt32(dgvUnit.CurrentRow.Cells[0].Value), false);
        ...
        //txtUnitName_KeyDown to Recall Method btnSave_Click(sender, e); 
        private void txtUnitName_KeyDown(object sender, KeyEventArgs e)
        {
            //if e.KeyCode == Keys.Enter
            if (e.KeyCode == Keys.Enter)
            {
                //Add New Unit Form to ItemUnitForm Table
                AddItemUnitForm();
            }
        }
