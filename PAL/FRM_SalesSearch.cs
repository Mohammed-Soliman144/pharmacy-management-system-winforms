...
{
    public partial class FRM_SalesSearch : Form
    {
        ...
        private static FRM_SalesSearch SaleSearchFRM;

        //Create SaleAccess_FormClosed to recall it when close form
        private static void SaleSearchAccess_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        public static FRM_SalesSearch SaleSearchAccess_Property
        {
            //used getter to return value of FRM_SalesSearch 
            get
            {
                //if created instance or field is null
                if (SaleSearchFRM == null)
                {
                    //Create New Instance From FRM_Items by initialize it to field
                    SaleSearchFRM = new FRM_SalesSearch();
                    //Generate Event of Form Closed and Delegate Method SaleSearchAccess_FormClosed to it
                    SaleSearchFRM.FormClosed += new FormClosedEventHandler(SaleSearchAccess_FormClosed);
                }
                //Return the value of field
                return SaleSearchFRM;
            }
        }
        ...
        /// </summary>
        public FRM_SalesSearch()
        {
            InitializeComponent();
            //Check if Field you are created is null so intialize value of FRM_SalesSearch to it 
            if (SaleSearchFRM == null) SaleSearchFRM = this;
        ...
        {
            double totalB = 0, totalG = 0, netG = 0;
            //Use for Loop To Get Calculations
            for (int i = 0; i < dgvDetailsSearch.Rows.Count; i++)
            {
                //Get ItemTotalSalePrice
                totalB += Convert.ToDouble(dgvDetailsSearch.Rows[i].Cells["ItemTotalSalePrice"].Value);
                //Get ItemTotalSalePrice
                totalG += Convert.ToDouble(dgvDetailsSearch.Rows[i].Cells["ItemTotalSalePrice"].Value);
                //Get ItemNetSalePrice
                netG += Convert.ToDouble(dgvDetailsSearch.Rows[i].Cells["ItemNetSalePrice"].Value);
            }
            ...
            lblSaleBillNetG.Text = netG.ToString();
        }
        ...
        private void btnExit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (SaleSearchFRM != null) SaleSearchFRM = null;
                //Close Form
                this.Close();
            }
            catch (Exception err)
                ...
                if (chBoxSearchDate.Visible == true && chBoxSearchDate.Checked == false)
                {
                    ...
                    switch (comboSearchWay.Text)
                    {
                        ...
                        case "نوع الفاتورة":
                            ...
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleBillClass.SearchSaleBill("SaleBillPayType", comboSearchKey.Text, false, "", ""), "رقم الفاتورة");
                            break;

                        case "اسم المخزن":
                            // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                            // Set DataTable is DataSource of DataGridView search by StoreName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleBillClass.SearchSaleBill("StoreName", comboSearchKey.Text, false, "", ""), "رقم الفاتورة");
                            break;

                        case "اسم الفرع":
                            // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                            // Set DataTable is DataSource of DataGridView search by BranchName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleBillClass.SearchSaleBill("BranchName", comboSearchKey.Text, false, "", ""), "رقم الفاتورة");
                            break;

                        case "اسم العميل":
                            // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                            // Set DataTable is DataSource of DataGridView search by CustomerFullName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleBillClass.SearchSaleBill("CustomerFullName", comboSearchKey.Text, false, "", ""), "رقم الفاتورة");
                            break;

                        case "باركود الصنف": //+dtp
                            // Declare String[] array to initialize values to it
                            //string[] strArray = new string[] { "باركود الاول للصنف", "الباركود الثانى للصنف", "الباركود الثالث" };

                            // Set DataTable is DataSource of DataGridView search by ItemBarcode1
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleDetailsClass.SearchSaleBillDetails("ItemBarcode1", txtSearchKey.Text, false, "", ""), "رقم الفاتورة");
                            break;

                        case "اسم الصنف":

                            // Initialize values to Local String[] array
                            //string[] strArray = new string[] { "اسم الصنف", "الصنف بالانجليزى" };

                            //Set DataTable is DataSource of DataGridView search by ItemName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleDetailsClass.SearchSaleBillDetails("ItemName", txtSearchKey.Text, false, "", ""), "رقم الفاتورة");
                            break;

                        case "الشركة المنتجة":
                            // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                            // Set DataTable is DataSource of DataGridView search by CompanyName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleDetailsClass.SearchSaleBillDetails("CompanyName", txtSearchKey.Text, false, "", ""), "رقم الفاتورة");
                            break;

                        case "المجموعة العلمية":
                            // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                            // Set DataTable is DataSource of DataGridView search by GroupName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleDetailsClass.SearchSaleBillDetails("GroupName", txtSearchKey.Text, false, "", ""), "رقم الفاتورة");
                            break;
               ...
               else if (chBoxSearchDate.Visible == true && chBoxSearchDate.Checked == true)
               {
                    ...
                    switch (comboSearchWay.Text)
                    {
                        case "نوع الفاتورة":
                            // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                            // Set DataTable is DataSource of DataGridView search by SaleBillPayType
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleBillClass.SearchSaleBill("SaleBillPayType", comboSearchKey.Text, true, dtpDateFrom.Value.ToString("yyyy-MM-dd"), dtpDateTo.Value.ToString("yyyy-MM-dd")), "رقم الفاتورة");
                            break;
                        ...
                        case "تاريخ الفاتورة":
                            ...
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleBillClass.SearchSaleBill("SaleBillInvoiceDate", "", true, dtpDateFrom.Value.ToString("yyyy-MM-dd"), dtpDateTo.Value.ToString("yyyy-MM-dd")), "رقم الفاتورة");
                            break;

                        case "اسم المخزن":
                            // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                            // Set DataTable is DataSource of DataGridView search by StoreName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleBillClass.SearchSaleBill("StoreName", comboSearchKey.Text, true, dtpDateFrom.Value.ToString("yyyy-MM-dd"), dtpDateTo.Value.ToString("yyyy-MM-dd")), "رقم الفاتورة");
                            break;

                        case "اسم الفرع":
                            // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                            // Set DataTable is DataSource of DataGridView search by BranchName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleBillClass.SearchSaleBill("BranchName", comboSearchKey.Text, true, dtpDateFrom.Value.ToString("yyyy-MM-dd"), dtpDateTo.Value.ToString("yyyy-MM-dd")), "رقم الفاتورة");
                            break;

                        case "اسم العميل":
                            // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                            // Set DataTable is DataSource of DataGridView search by CustomerFullName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleBillClass.SearchSaleBill("CustomerFullName", comboSearchKey.Text, true, dtpDateFrom.Value.ToString("yyyy-MM-dd"), dtpDateTo.Value.ToString("yyyy-MM-dd")), "رقم الفاتورة");
                            break;

                        case "باركود الصنف": //+dtp
                            // Set DataTable is DataSource of DataGridView search by ItemBarcode1
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleDetailsClass.SearchSaleBillDetails("ItemBarcode1", txtSearchKey.Text, true, dtpDateFrom.Value.ToString("yyyy-MM-dd"), dtpDateTo.Value.ToString("yyyy-MM-dd")), "رقم الفاتورة");
                            break;

                        case "اسم الصنف"://+dtp
                            // Set DataTable is DataSource of DataGridView search by ItemName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleDetailsClass.SearchSaleBillDetails("ItemName", txtSearchKey.Text, true, dtpDateFrom.Value.ToString("yyyy-MM-dd"), dtpDateTo.Value.ToString("yyyy-MM-dd")), "رقم الفاتورة");
                            break;

                        case "الشركة المنتجة"://+dtp
                            // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                            // Set DataTable is DataSource of DataGridView search by CompanyName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleDetailsClass.SearchSaleBillDetails("CompanyName", txtSearchKey.Text, true, dtpDateFrom.Value.ToString("yyyy-MM-dd"), dtpDateTo.Value.ToString("yyyy-MM-dd")), "رقم الفاتورة");
                            break;

                        case "المجموعة العلمية"://+dtp
                            // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                            // Set DataTable is DataSource of DataGridView search by GroupName
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleDetailsClass.SearchSaleBillDetails("GroupName", txtSearchKey.Text, true, dtpDateFrom.Value.ToString("yyyy-MM-dd"), dtpDateTo.Value.ToString("yyyy-MM-dd")), "رقم الفاتورة");
                            break;
               ...
               else if (chBoxSearchDate.Visible == false)
               {
                    ...
                    switch (comboSearchWay.Text)
                    {
                        case "كود الفاتورة":
                            ...
                            dgvHeadSearch.DataSource = RemoveDuplicatedRows(SaleBillClass.SearchSaleBill("SaleBillCustomID", txtSearchKey.Text, false, "", ""), "رقم الفاتورة");
                            break;
                ...
                AddNewRows(dgvHeadSearch, "SaleBillSerial");
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                RPT.FRM_SalesViewerRpt SalePillViewerFrm = new RPT.FRM_SalesViewerRpt();
                //Create New Instance From RPT.SalesRpt is Report
                RPT.SalesRpt SalePillReport = new RPT.SalesRpt();
                //Initialize ReportSource of CrystalReportViewer is Instance which you are created ItemReport
                SalePillViewerFrm.crystalRptViewerSales.ReportSource = SalePillReport;
                ...
                SalePillReport.SetParameterValue("@SearchKey", Convert.ToInt32(dgvHeadSearch.CurrentRow.Cells["SaleBillID"].Value));

                //Refresh crystalRptViewerSales
                SalePillViewerFrm.crystalRptViewerSales.Refresh();
                //Show Form Report Viewer in mode Dialog to control it as first
                SalePillViewerFrm.ShowDialog();
        ...
        private void dgvHeadSearch_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    // Recall DataTable Fucation from BAL(Business Access Layer) to Search SaleBill by txtSearch.Text
                    // Set DataTable is DataSource of DataGridView search by SaleBillID
                    dgvDetailsSearch.DataSource = SaleDetailsClass.SearchSaleBillDetails("SaleBillID", dgvHeadSearch.CurrentRow.Cells["SaleBillID"].Value.ToString(), false, "", "");

                    //Add Serial To SaleDetailSerial of dgvDetailsSearch
                    AddNewRows(dgvDetailsSearch, "SaleDetailSerial");
                    //GetCalculations
                    GetCalculations();
                }
                //if the index of row greater than or equal zero and index of button column Print btn is 
                else if (e.RowIndex >= 0 && e.ColumnIndex == 19)
                {
                    //Change Cursor to wait Cursor
                    this.Cursor = Cursors.WaitCursor;
                    //Create New Instance From  RPT.FRM_SalesViewerRpt is Form Viewer
        ...
        private void dgvDetailsSearch_ColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                {
                    AddNewRows(dgvDetailsSearch, "SaleDetailID");
                }
            }
            catch (Exception err)
            {
