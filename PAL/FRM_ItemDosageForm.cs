...
{
    public partial class FRM_ItemDosageForm : Form
    {

        #region Public Declarations
        //Create New Instance From BAL.CLS_ItemDosageForm Business Access Layer
        readonly BAL.CLS_ItemDosageForm DosageFormClass = new BAL.CLS_ItemDosageForm();
        ...
        private void InitializeDGV()
        {
            ...
            dgvDosage.MultiSelect = false;
            //Intialize SelectionMode of DGV is DataGridViewSelectionMode.FullRowSelect
            dgvDosage.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            //Intialize AllowUserToAddRows of DGV is False
            dgvDosage.AllowUserToAddRows = false;
            //Intialize AllowUserToDeleteRows of DGV is False
            dgvDosage.AllowUserToDeleteRows = false;
            //Intialize AllowUserToOrderColumns of DGV is False
            dgvDosage.AllowUserToOrderColumns = false;
            //Intialize AllowUserToResizeColumns of DGV is False
            dgvDosage.AllowUserToResizeColumns = false;
            //Intialize AutoSizeColumnsMode of DGV is DataGridViewAutoSizeColumnsMode.DisplayCells
            dgvDosage.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.DisplayedCells;
            //Intialize AutoSizeRowsMode of DGV is DataGridviewAutoSizeRowsMode.DisplayedCells
            dgvDosage.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.DisplayedCells;
            //Intialize AutoGenerateColumns of DGV is False
            dgvDosage.AutoGenerateColumns = false;
            //Intialize DataSource of DGV is DosageFormClass.ShowItemDosageFormTable();
            dgvDosage.DataSource = DosageFormClass.ShowItemDosageFormTable();
        }
        ...
        //AddItemDosageForm Method To Add New Dosage Form to ItemDosageForm Table
        private void AddItemDosageForm()
        {
            try
            {
                //Check all required fields are filled or not
                if (txtDosageName.Text == string.Empty)
                {
                ...
                //check Duplicate of item Dosage Form in datagridview by for loop
                for (int i = 0; i <= dgvDosage.Rows.Count - 1; i++)
                {
                    //if dgvDosage.Rows[i].Cells[1].Value.ToString() is equal txtDosageName.Text
                    if (dgvDosage.Rows[i].Cells[1].Value.ToString() == txtDosageName.Text)
                    {
                        //Warning Message Box
                        NotificationSMS.NotifyShow("هذا الشكل تم إضتافته من قبل", "تنبية",
                            FRM_Notifications.NotifyButtons.Ok, FRM_Notifications.NotifyIcons.Warning,
                            FRM_Notifications.NotifyTypes.NotifyBtn);
                        //Show Notification Message in Dialog Mode
                        NotificationSMS.ShowDialog();
                ...
                DosageFormClass.SaveDosageForm(txtDosageName.Text, IP_PCName, Program.UsrID, true, DateTime.Now.ToString("yyyy-MM-dd"),
        ...
        //dgvDosage_CellContentClick Method to Delete any record of ItemDosageForm Table
        private void dgvDosage_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.RowIndex >= 0 && e.ColumnIndex == 3)
                {
                    ...
                    //if user select Yes Button and Select Delete itemDosageForm From Database
                    if (NotificationSMS.NotifyResult == true && NotificationSMS.NotifyType == "Delete Record")
                    {
                        //Delete itemDosageForm From Database
                        DosageFormClass.DeleteDosageForm(Convert.ToInt32(dgvDosage.CurrentRow.Cells[0].Value));
                    ...
                    //if user select Yes Button and Select InActivate itemDosageForm From Database
                    else if (NotificationSMS.NotifyResult == true && NotificationSMS.NotifyType == "InActivate Record")
                    {

                        //Update Status to False in itemDosageForm Table
                        DosageFormClass.DeactivateDosageForm(Convert.ToInt32(dgvDosage.CurrentRow.Cells[0].Value), false);
        ...
        //txtDosageName_KeyDown to Recall Method btnSave_Click(sender, e); 
        private void txtDosageName_KeyDown(object sender, KeyEventArgs e)
        {
            //if e.KeyCode == Keys.Enter
            if (e.KeyCode == Keys.Enter)
            {
                //Add New Dosage Form to ItemDosageForm Table
                AddItemDosageForm();
            }
        }
